...
import net.scalytica.gitlab.models.{Branch, CommitSha, ProjectId}
  ...
  def prettyPrint(mr: MergeRequest): Unit = {
    ...
    val out =
      s"""
         ...
         !assignee        : ${mr.assignee.map(_.username).getOrElse("")}
  ...
  def prettyPrint(mrs: Seq[MergeRequest]): Unit = {
    val table = TablePrinter.format(Seq(headers) ++ mrs.map { mr =>
      Seq(
        ...
        mr.targetBranch,
        mr.sourceBranch,
        mr.author.username.value,
        mr.assignee.map(_.username.value).getOrElse(""),
        mr.labels.mkString(", "),
        mr.workInProgress,
